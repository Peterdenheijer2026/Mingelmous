...
export default async function PleinPage({ params }: { params: { slug: string } }) {
  ...
  let content: { general_text: string | null; program_data: unknown; image_paths: string[] | null } | null = null;
  try {
    ...
    // Supabase niet geconfigureerd of geen tabel
  }
  ...
  return (
    <div className="plein-page">
      <header className="header">
        <Link href="/" className="header__logo" aria-label="Naar hoofdpagina">
          <span className="header__brand-main" style={{ fontSize: "1.25rem", color: "var(--color-orange)" }}>Site</span>
        </Link>
      ...
      <main className="plein-page__main">
        ...
        {!content?.general_text && !hasProgramData && images.length === 0 && (
          <p className="plein-page__empty">Nog geen content voor dit plein. Log in op beheer om content toe te voegen.</p>
        )}
      </main>
